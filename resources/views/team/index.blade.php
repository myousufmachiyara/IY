                    ...
                    <table class="table table-bordered table-striped mb-0" id="datatable-default">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $u->username }}</td>
                                <td><span class="badge bg-secondary text-uppercase">{{ $roleLabels[$u->roles->first()?->name] ?? '—' }}</span></td>
                                <td>{{ $u->email }}<br><small class="text-muted">{{ $u->phone }}</small></td>
                                <td>
                                    @if($u->roles->first()?->name === 'sales_agent')
                                        {{ $u->sales_commission_percent }}% + ¥{{ number_format($u->sales_fixed_bonus) }}
                                    @elseif($u->roles->first()?->name === 'vendor_agent')
                                ...
                                <td>
                                    @can('team.edit')
                                        <a href="#" class="text-primary me-1" onclick="editUser({{ $u->id }})">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                    @endcan
                                    @can('team.delete')
                                        <form action="{{ route('team.destroy', $u) }}" method="POST" style="display:inline;"
                ...
                <form method="POST" action="{{ route('team.store') }}" onkeydown="return event.key != 'Enter';">
                    ...
                    <div class="card-body">
                        <div class="row form-group">
                            ...
                            <div class="col-lg-6 mb-2 add-economics add-economics-sales_agent d-none">
                                <label>Sales Commission %</label>
                                <input type="number" step="0.01" class="form-control" name="sales_commission_percent" value="15">
                            </div>
                            <div class="col-lg-6 mb-2 add-economics add-economics-sales_agent d-none">
                                <label>Fixed Bonus per Won Bid (¥)</label>
                                <input type="number" class="form-control" name="sales_fixed_bonus" value="0">
                            </div>
                            <div class="col-lg-6 mb-2 add-economics add-economics-vendor_agent d-none">
                                <label>Vendor Commission %</label>
                                <input type="number" step="0.01" class="form-control" name="vendor_commission_percent" value="7">
                            </div>
                            <div class="col-lg-6 mb-2 add-economics add-economics-vendor_agent d-none">
                ...
                <form method="POST" id="editForm" action="" onkeydown="return event.key != 'Enter';">
                    ...
                    <div class="card-body">
                        <div class="row form-group">
                            ...
                            <div class="col-lg-6 mb-2 edit-economics edit-economics-sales_agent d-none">
                                <label>Sales Commission %</label>
                                <input type="number" step="0.01" id="edit_sales_commission_percent" class="form-control" name="sales_commission_percent">
                            </div>
                            <div class="col-lg-6 mb-2 edit-economics edit-economics-sales_agent d-none">
                                <label>Fixed Bonus per Won Bid (¥)</label>
                                <input type="number" id="edit_sales_fixed_bonus" class="form-control" name="sales_fixed_bonus">
                            </div>
                            <div class="col-lg-6 mb-2 edit-economics edit-economics-vendor_agent d-none">
                                <label>Vendor Commission %</label>
                                <input type="number" step="0.01" id="edit_vendor_commission_percent" class="form-control" name="vendor_commission_percent">
                            </div>
                            <div class="col-lg-6 mb-2 edit-economics edit-economics-vendor_agent d-none">
                                <label>Yard / Auction Location</label>
                                <input type="text" id="edit_vendor_location" class="form-control" name="vendor_location">
                            </div>
                        </div>
                    </div>
                    <footer class="card-footer">
...
function toggleEconomics(prefix) {
    ...
    $('.' + prefix + '-economics-' + role).removeClass('d-none');
}
...
function editUser(id) {
    fetch('/team/' + id + '/edit')
        ...
        .catch(err => {
            ...
            alert('Could not load member data. Please try again.');
        });
}
</script>
