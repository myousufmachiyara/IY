@extends('layouts.app')
@section('title', 'Accounting | Party Ledger')
@section('content')
<div class="row"><div class="col"><section class="card">
    <header class="card-header"><h2 class="card-title">Accounting</h2></header>
    @include('accounting._tabs', ['active' => 'party_ledger'])
    <div class="card-body">
        <form method="GET" class="row g-2 mb-3">
            <div class="col-md-2">
                <select name="party_type" class="form-control select2-js" onchange="this.form.submit()">
                    <option value="customer" @selected($type==='customer')>Customer</option>
                    <option value="vendor" @selected($type==='vendor')>Vendor</option>
                </select>
            </div>
            <div class="col-md-3">
                <select name="account_id" class="form-control select2-js" onchange="this.form.submit()">
                    <option value="">Select {{ ucfirst($type) }}</option>
                    @foreach($accounts as $a)<option value="{{ $a->id }}" @selected($selected?->id == $a->id)>{{ $a->name }}</option>@endforeach
                </select>
            </div>
        ...
        <div class="table-scroll">
            <table class="table table-bordered table-striped mb-0">
                ...
                <tbody>
                    ...
                    @empty<tr><td colspan="5" class="text-center text-muted py-3">No entries in this range.</td></tr>@endforelse
                </tbody>
            </table>
        </div>
        @else
        <p class="text-muted">Select a {{ $type }} above to view their ledger.</p>
        @endif
    </div>
</section></div></div>
@endsection
